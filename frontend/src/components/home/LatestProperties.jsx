...
const LatestProperties = () => {
    ...
    useEffect(() => {
        const loadProperties = async () => {
            try {
                // Get properties specifically marked for homepage
                const BASE = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
                const response = await fetch(`${BASE}/properties/?status=approved&homepage=true`);
                const data = await response.json();
                const list = Array.isArray(data.properties) ? data.properties : [];

                // Format for child component expectations
                ...
                const sorted = formatted
                    .sort((a, b) =>
                        new Date(b.approved_at || b.approvedAt || b.created_at || 0) -
                        new Date(a.approved_at || a.approvedAt || a.created_at || 0)
                    )
                    .slice(0, 12); // Show up to 12 properties
                ...
                console.error('LatestProperties load error:', err);
            }
        };
